...
class Graph:
    ...
    def blocking(self, y: Vertex, delta, filename):
        ...
        while e != None:
            ...
            walk1 = self.walk(y, u, filename)
            traverse = self.traverse(e, filename)
            # print("walk1 traverse " + str(float(walk1) + float(traverse)))
            cost += walk1 + traverse
            cost += self.blocking(v, delta, filename)
            # cost += self.walk(v, y, filename)
            walk2 = self.walk(v, y, filename)
            # print("walk2", str(float(walk2)))
            # cost += walk2
            e = self.get_satisfying_edge(y, delta, old_blocked_edges)
